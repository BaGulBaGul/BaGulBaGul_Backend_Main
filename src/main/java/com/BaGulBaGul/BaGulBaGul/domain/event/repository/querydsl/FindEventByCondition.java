...
public interface FindEventByCondition {
     ...
     * 조건이 복잡해서 fetch join을 하기 위해 id를 반환한다(컬렉션 정합성). 서비스에서 이를 where in을 통해 fetch join해줘야 한다.
     */
    @Transactional
    EventIdsWithTotalCountOfPageResult getEventIdsWithFetchJoinByConditionAndPageable(

...
public class TagRankingUpdateJobScheduler {
    private final JobLauncher jobLauncher;
    private final Job tagRankingUpdateJob;

    @Value("${ranking.tag.aggregate-period}")
    private int AGGREGATE_PERIOD;

    @Scheduled(cron = "${ranking.tag.batch.scheduler.cron}")
    public void run() {
        ...
        log.info("작업");
        try {
            jobLauncher.run(tagRankingUpdateJob, jobParameters);

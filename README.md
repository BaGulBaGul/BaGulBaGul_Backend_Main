...
바글바글은 각종 이벤트(패스티벌, 지역행사, 파티)에 대한 정보를 공유하고 함께 참여하거나 직접 파티를 주최하는 커뮤니티입니다.
...
# 기술적 특징
### 게시판 기능
- event(패스티벌, 지역행사, 파티) 정보 게시물을 작성하거나, 어떤 event에 함께 참여할 사람을 모집하기 위해 recruitment(모집) 게시물을 작성할 수 있습니다.
- 각 게시물에는 댓글, 대댓글을 작성할 수 있습니다.  
- 게시물, 댓글, 대댓글에 대한 기본적인 crud, 좋아요 누르기, 신고 시스템 등 게시판의 기본 기능을 구현했습니다.
- QueryDSL을 이용해 복잡한 동적 조회 쿼리를 작성했습니다. 제목, 작성자, 카테고리, 태그, 인원수, 장소, 시간 등 다양한 조건의 조합으로 이벤트와 모집글의 상세 조건 검색이 가능합니다.
<br>

### 공통 도메인 분리
- event와 recruitment에서 게시글 기능이 공통으로 사용되었고 코드 중복이 있었습니다.
따라서 게시글 코드를 post(게시글) 도메인으로 분리해서 코드를 재사용하고 유지보수성을 높였습니다.
- 게시글 서비스에서 게시글 이미지 서비스, 댓글 서비스, 검색 서비스, 알람 서비스 등 다양한 서비스와 연결됩니다.
- 게시글에 대한 QueryDSL 쿼리 생성 코드를 분리해서 이벤트와 모집글의 쿼리 생성에 재사용하고 코드 중복을 제거했습니다.
- 공지사항 등의 게시글 기능이 필요한 새로운 기능으로 확장이 쉬워졌습니다.
<br>
...
### 인증 
- 카카오 oauth나 id/pw를 이용해 유저를 인증합니다.
- 회원가입하지 않은 유저라면 회원가입 페이지로 이동 후 추가정보를 작성하고 회원가입 합니다.
- 회원가입한 유저라면 AT(access token), RT(refresh token)를 jwt로 쿠키에 발급합니다.
  - AT는 userId, role 정보를 가지고 RT는 userId 정보만 가집니다. 
  - AT, RT를 쿠키에 http only, secure로 저장합니다. 
  - RT는 쿠키의 path를 재발급 경로로만 한정해서 보안을 높였습니다.
  - AT, RT 모두 http only 쿠키를 사용하기 때문에 xss공격에는 상대적으로 안전합니다.
  - csrf 공격을 막기 위해 double-submit-cookie 방식의 csrf 토큰을 도입했습니다.
- JWT 발급 시 AT의 jti와 RT의 jti 정보를 redis에 저장하고 재발급시 검증합니다.
  - 발급 당시의 AT, RT 정보가 일치해야 재발급이 가능하며, 재발급을 시도한 AT, RT는 성공 여부에 상관 없이 redis에서 삭제됩니다.
  - 따라서 재발급 시에는 AT, RT를 전부 재발급하고 기존 AT, RT는 재발급에 재사용이 불가능합니다.
- 실시간 알람서버 등의 다른 서버와 jwt를 공유합니다.
- 사용자 요청을 받으면 OncePerRequestFilter가 쿠키에서 jwt의 user id와 role을 추출해서 security context에 저장하고 인증에 사용됩니다.
<br>

### 유저의 역할, 권한
- 바글바글 서비스에서 다양한 작업을 수행하기 위해 권한이 필요합니다.
- 유저는 여러 역할을 가지고 있고, 역할은 여러 권한을 가지고 있습니다.
- 각 역할이 갖는 권한은 성능을 위해 각 인스턴스가 내부 메모리에 캐시하고 있습니다.
  - 어떤 인스턴스에서 역할-권한 캐시에 변경이 일어난다면 redis pub/sub을 이용해 이벤트를 전파하고 모든 인스턴스가 db에서 변경사항을 다시 로드합니다. 
- 유저의 역할 정보는 AT에 저장되어서 요청 시 filter에서 db에 질의하지 않고 즉시 처리됩니다.
<br>

### 관리자 기능
- 유저의 계정 정지를 관리할 수 있습니다.
- 관리자용 유저를 생성하거나 기존 유저에게 역할을 부여할 수 있습니다.
- 역할을 생성하고 권한을 부여할 수 있습니다.
- 게시글, 댓글 등을 관리하고 삭제할 수 있습니다.
- 신고내역을 확인하고 처리할 수 있습니다.
- 이 외에도 카테고리를 편집하는 등의 필수적인 관리자 기능을 수행할 수 있습니다.
<br>

### 이미지 업로드
- 유저가 업로드 서비스에 이미지를 업로드하면 aws s3에 업로드되고 임시 자원인 TempResource로 등록됩니다.
- 사용자가 게시물이나 프로필 이미지 등에 이미지를 등록하면 게시글 이미지 서비스, 유저 이미지 서비스 등에서 영구 자원인 Resource로 전환합니다.
...
### 알람
- 댓글, 답글이 달리거나 다른 유저에게 좋아요를 받는 등 다양한 상황에서 유저에게 알람을 보냅니다. 유저는 마이페이지에서 알람을 확인 가능합니다.
<br>
...
- 비동기 실행, 트랜젝션 제어, redis와의 상호작용 등 복잡한 상황에 대한 테스트를 작성했습니다.
<br>
